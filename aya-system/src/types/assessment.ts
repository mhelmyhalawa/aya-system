// نوع التقييم
export type AssessmentType = 'juz' | 'half_quran' | 'full_quran' | 'periodic';

// خيارات أنواع التقييم
export const assessmentTypeOptions = [
  { value: 'juz', label: 'اختبار جزء' },
  { value: 'half_quran', label: 'اختبار نصف القرآن' },
  { value: 'full_quran', label: 'اختبار القرآن كامل' },
  { value: 'periodic', label: 'اختبار دوري' }
];
...
// تنسيق نطاق التقييم
export function formatAssessmentRange(assessment: Assessment): string {
  if (!assessment.from_surah || !assessment.to_surah) {
    return 'غير محدد';
  }

  const fromSurah = quranSurahs.find(s => s.number === assessment.from_surah);
  const toSurah = quranSurahs.find(s => s.number === assessment.to_surah);
  ...
  if (fromSurah.number === toSurah.number) {
    return `سورة ${fromSurah.name} (${assessment.from_ayah} - ${assessment.to_ayah})`;
  }

  return `من سورة ${fromSurah.name} آية ${assessment.from_ayah} إلى سورة ${toSurah.name} آية ${assessment.to_ayah}`;
}
...
// واجهة التقييم كما هي في قاعدة البيانات
...
export function getSurahName(surahNumber: number): string {
  ...
  return surah ? surah.name : 'غير معروف';
}
