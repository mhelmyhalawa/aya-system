 ...
 * @param forceServiceKey إجبار استخدام مفتاح الخدمة
 */
export const updateRecord = async (table, query, data, forceServiceKey = false) => {
  try {
    ...
    // للجداول غير المحمية، نستخدم JWT token إذا كان متوفراً
    const userToken = getUserJWTToken();
    ...
    // إذا وصلنا إلى هنا، نستخدم مفتاح الخدمة كخطة بديلة
    console.log('SupabaseREST: محاولة تحديث السجل باستخدام مفتاح الخدمة كخطة بديلة');
    ...
    const fallbackResponse = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${query}`, {
      method: 'PATCH',
      headers: fallbackHeaders,
      body: JSON.stringify(data)
    });
    ...
    if (fallbackResponse.ok) {
      const result = await fallbackResponse.json();
      console.log(`SupabaseREST: تم تحديث السجل بنجاح في جدول ${table} باستخدام مفتاح الخدمة (خطة بديلة)`);
      return { success: true, data: result };
    }
    
    const fallbackErrorText = await fallbackResponse.text();
    console.error(`SupabaseREST: فشل تحديث السجل في جدول ${table}:`, fallbackErrorText);
 ...
 * @param forceServiceKey إجبار استخدام مفتاح الخدمة
 */
export const findRecord = async (table, query, authMode = 'auto', forceServiceKey = false) => {
  try {
    ...
    if (isProtectedTable) {
      ...
      const errorText = await response.text();
      console.error(`SupabaseREST: فشل البحث في جدول ${table}:`, {
        status: response.status,
        statusText: response.statusText,
        error: errorText
      });
